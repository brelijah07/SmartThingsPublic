...
def pageSettings(){
    dynamicPage(name: "pageSettings", uninstall: false){
        ...
        section ("Advanced") {
            href "pageCustomDevices", title: "Device Specific Commands", description: none, state: (nestCMD || stelproCMD || sonosCMD || MyEcobeeCMD ? "complete" : null)
...
def pageCustomDevices(){
    dynamicPage(name: "pageCustomDevices", uninstall: false){
		section("Device Specific Commands"){
            input "MyEcobeeCMD", "bool", title: "MyEcobee Specific Thermostat Modes\n(Home/Away/Sleep/Resume Program)\nAnd Specific Tips (Get Tips/Restart Tips)", defaultValue: false             
...
def processList(){
    ...
    if (listType=="thermostats" || listType=="thermostat"){outputTxt = tstats && tstats.size()>1 ? "The available thermostats are: " +  getList(tstats) + ". "
        ...
        if (op=="status") {
            if (type == "temperature"){
                ...
                if (otherStatus){
                    ...
                    if (MyEcobeeCMD && STdevice.currentValue('currentProgramId') =~ /home|away|sleep/ ){
                		result += " This thermostat's comfort setting is set to "
                    	result += "${STdevice.currentValue('currentProgramId')}. "
					}
                    if (MyEcobeeCMD && STdevice.currentValue('climateName') =~ /home|away|sleep/ ){
             		result += " This thermostat's comfort setting is set to "
            ...
            if (type == "thermostat"){
                ...
                if (op == "on" || op=="off") {
                    ...
                    if ((param =="home" || param =="away" || param =="sleep") && MyEcobeeCMD) {
                        STdevice.supportedCommands.each {comm ->
                            if(comm.name == "setThermostatProgram") {
                                result = "I am setting the ${STdevice} to '" + param + "'. "
                                STdevice.setThermostatProgram("${param.capitalize()}")
                            }
                            else if(comm.name == "setClimate") {
                                result = "I am setting the ${STdevice} to '" + param + "'. "
                                STdevice.setClimate("","${param.capitalize()}")
                            }
                        }
                    }
...
def groupResults(num, op, colorData, param, mNum){   
    ...
    if (groupType=="switch"){
        ...
        if (op == "on" || op=="off") {
            ...
            if ((param =="home" || param =="away" || param =="sleep") && parent.MyEcobeeCMD) {
                result = "I am setting the ${noun} to '" + param + "'. "
                settings."groupDevice${groupType}".each { -> myDevice
                    myDevice.supportedCommands.each {comm ->
                        if(comm.name == "setThermostatProgram") {
                            myDevice.setThermostatProgram("${param.capitalize()}")
                        }else if(comm.name == "setClimate") {
                            myDevice.setClimate("","${param.capitalize()}")
                        }
                    }
                }
            }
...
def macroTypeDesc(){
    ...
    if (macroType =="Voice" && (voicePre || voiceSwitch || voiceDimmer || voiceDoorSensors || voiceDoorControls || voiceDoorLocks || voiceTemperature ||  
        ...
        voiceBattery || voicePost || voiceMode || voiceSHM || voicePower)) desc= "Voice Report CONFIGURED - Tap to edit" 
	if (macroType =="Group" && groupType && settings."groupDevice${groupType}") {
        ...
        if (parent.MyEcobeeCMD && groupType=="thermostat") customAck = "- Accepts My Ecobee 'Home'/'Away'/'Sleep'/'Resume Program'/'Get Tips/'Erase Tips' commands" + customAck
...
def setupData(){
    ...
    if (tstats && (nestCMD || MyEcobeeCMD)) PARAMS<<"home"<<"away"
    if (tstats && MyEcobeeCMD) PARAMS<<"sleep"<<"resume program"<<"get tips"<<"erase tips"
    if (sonosCMD && speakers && sonosMemoryCount){
    	def memCount = sonosMemoryCount as int
    	for (int i=1; i<memCount+1; i++){
...
private def textVersion() {
    def version = "SmartApp Version: 2.1.3 (08/28/2016)", lambdaVersion = state.lambdaCode ? "\n" + state.lambdaCode : ""
